...
function Davlatlar() {
  ...
  useEffect(() => {
    api
      .get(`/countries?limit=${button}&skip=5`)
      .then((response) => {
  ...
  return (
    <div>
      <div className="max-w-[1280px] mx-auto px-2">
        <div className="my-[20px] flex flex-wrap gap-2 px-2 justify-between items-center ">
          ...
          <div className="relative z-10">
            <div
              onClick={off}
              className="rounded-[5px] div-10 h-[58px] px-[20px] bg-base-100 flex items-center justify-between w-52 cursor-pointer"
            >
              <h2>{categori}</h2>
              <img
                className="w-4 h-4"
                src="https://cdn-icons-png.flaticon.com/512/5727/5727082.png"
                alt="Dropdown icon"
              />
            </div>
            {categoriCorsatish && (
              <div className="drop-shadow-md rounded-[5px] absolute top-20 bg-base-100 w-52 gap-2 flex flex-col items-start">
                <button
                  onClick={() => {
                    setCategori("Filter by Region");
                    setCategoriCorsatish(false);
                  }}
                  className="w-full text-start hover:bg-base-200 py-1 px-5"
                >
                  Filter by Region
                </button>
                <button
      ...
      {categoriCorsatish && (
        <div
          onClick={() => setCategoriCorsatish(false)}
